    ...
    def __init__(self, path, transform, resolution=256):
        ...
        with self.env.begin(write=False) as txn:
            ...
            if length_bytes is not None:
                # If the value exists, decode it
                self.length = int(length_bytes.decode("utf-8"))
            else:
                # Handle the case where the key does not exist
                # For example, you could set a default value or raise an exception
                self.length = 0  # Set a default value or handle it appropriately
                print("Warning: 'length' key not found in the database.")

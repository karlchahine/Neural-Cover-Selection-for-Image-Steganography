...
class LISO(object):
    ...
    def _encode_decode_robust(self, cover, std_dev, quantize=False, payload=None, init_noise=False, verbose=False):
        if payload is None:
            payload = self._random_data(cover)
        generated, grads, ptbs = self.encoder(cover, payload, init_noise=init_noise, verbose=verbose)
        if quantize and not self.jpeg:
        ...
        noisy_generated_list = [tensor + torch.randn_like(tensor) * std_dev for tensor in generated]

        decoded = [self._decoder(x) for x in noisy_generated_list]
